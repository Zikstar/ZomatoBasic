...
class LandingFragment: Fragment() {
    ...
    fun setupViewModelObservers(){
        viewModel.locationData.observe(viewLifecycleOwner, Observer { wrappedLocationResponse ->
            if (wrappedLocationResponse!= null && wrappedLocationResponse is ResultWrapper.Success){
                handleSuccessLocationResponse(wrappedLocationResponse)
            }else{
        ...
        viewModel.locationDetailsLiveData.observe(viewLifecycleOwner, Observer {
            if(it != null && it is ResultWrapper.Success){
                handleSuccesLocationDetailsResponse(it)
            }else{
                Toast.makeText(context, "There Was an error", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun handleSuccesLocationDetailsResponse(it: ResultWrapper.Success<LocationDetails>) {
    ...
    private fun handleSuccessLocationResponse(wrappedLocationResponse: ResultWrapper.Success<LocationSuggestionResponse>) {
